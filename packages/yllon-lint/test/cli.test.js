...
describe(`'fix' command`, () => {
  const dir = path.resolve(__dirname, './testExamples/examples');
  const outputFilePath = path.resolve(dir, './temp/temp.js');
  const errorFileContent = fs.readFileSync(path.resolve(dir, './semiErrored.js'), 'utf8');
  const expectedFileContent = fs.readFileSync(path.resolve(dir, './semiExpected.js'), 'utf8');

  beforeEach(() => {
    fs.outputFileSync(outputFilePath, errorFileContent, 'utf8');
  });

  test('should examples problematic code', async () => {
    await cli(['fix'], {
